  ...
  <v-container>
    <v-container fluid grid-list-xl>
        ...
        <v-dialog v-model="deleteAllDialog" max-width="500px">
          <v-card>
            ...
            <v-card-text>
              ...
              >You are about to delete your selection, are you sure?</span>
            </v-card-text>

            <v-card-actions>
              <v-btn text v-on:click="deleteAll(); deleteAllDialog=false;" color="red">Yes</v-btn>
              <v-btn text v-on:click="deleteAllDialog=false" color="blue">No</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-spacer />
      ...
      <v-layout column wrap v-if="myselection.length !== 0">
        <v-list>
          <v-list-item v-for=" characterName in myselection" :key="characterName.index">
            <v-list-item-icon>
            ...
            <v-list-item-action>
              <v-btn icon>
                <v-icon
                  v-on:click="removeFromMySelection(characterName)"
                  color="red lighten-1"
                >mdi-delete</v-icon>
              </v-btn>
            </v-list-item-action>
...
export default {
  ...
  methods: mapActions(["removeFromMySelection", "deleteAll"]),
