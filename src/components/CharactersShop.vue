  ...
  <v-container>
    <v-container fluid grid-list-xl>
      ...
      <v-row cols="12" sm="6" md="3">
        <v-text-field v-model="search" label="Solo" placeholder="Search" append-icon="search" solo></v-text-field>
      </v-row>
          ...
          <v-hover>
            <template v-slot:default="{ hover }">
              <v-card
                hover
                class="pointer"
                v-on:mouseenter="mouseOnCard(index)"
                v-on:mouseleave="mouseOutCard()"
              >
                <v-card-title>
                ...
                <v-card-text>
                  ...
                  <span v-else>Gender: {{character.gender}}</span>
                  <br />
                ...
                <v-row>
                  <v-fade-transition>
                    <v-overlay v-if="selectedIndexCard===index" absolute color="grey">
                      <v-btn
                        v-if="!isAlreadyAdded(character.name)"
                        v-on:click="addToMySelection(character.name)"
                        color="green"
                      >Add to my selection</v-btn>
        ...
        <v-card class="mt-4 pa-3">
          <v-layout column justify-center align-center>
            <v-flex class="pa-2" md8>
              <v-btn :disabled="true">No Result</v-btn>
            </v-flex>
          </v-layout>
        </v-card>
      </v-layout>
